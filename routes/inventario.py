...
def _historial(repuesto_id, ant, nuevo, accion):
    db.session.add(HistorialStock(repuesto_id=repuesto_id, stock_anterior=ant, stock_nuevo=nuevo, accion=accion))
...
def _stats():
    ...
    return dict(
        ...
        inversion=sum(p.p_costo * p.stock for p in todos),
...
def subir_masivo():
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if p:
                ant = p.stock; p.stock += int(row['stock']); p.p_costo = float(row['p_costo']); p.p_venta = float(row['p_venta'])
                _historial(p.id, ant, p.stock, 'AGREGADO')
            else:
                p = Repuesto(nombre=nom, p_costo=float(row['p_costo']), p_venta=float(row['p_venta']), stock=int(row['stock']))
                db.session.add(p); db.session.flush(); _historial(p.id, 0, int(row['stock']), 'AGREGADO')
